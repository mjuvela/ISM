...
void __kernel SimulateBG(const    float SEED,       // [ 0]
			 ...
			 __global float *A_ipFR,    // [ 5]  A_ipFR[NC, NT, NF]
			 __global float *A_ipET,    // [ 6]  A_ipET[NC, NT]
			 __global float *A_ipKABS,  // [ 7]  A_ipKABS[NC, NF]
			 __global float *A_ipKSCA,  // [ 8]  ipSCA[NC, NF]
			 __global float *A_ipG,     // [ 9]  ipG[NC, NF]
			 __global float *ABS,       // [10]  ABS[NC]
			 __global float *DABS,      // [11]  DABS[NC]
			 __global float *A_E0,      // [14]  A_E0[NC]
			 __global float *A_KE       // [15]  A_KE[NC]
			) {
   // constants NT, NF
...
#endif
   KABS      =  A_ipKABS[IND*NF+ifreq] ;
   KSCA      =  A_ipKSCA[IND*NF+ifreq] ;
   G         =  A_ipG[   IND*NF+ifreq] ;         
...
# endif
...
#endif
	    iE        =  clamp((int)(round(log(Eabs/A_E0[IND])/log(A_KE[IND]))), 0, NT-1) ;
	    T         =  A_ipET[IND*NT+iE] ;          // convert energy to temperature
	    ...
	    F         =  A_ipFR[IND*NT*NF+(int)round(iT*NF+NF*Rand(&rng))] ; // random frequency from the lookup table
...
void __kernel SimulatePS(const    float  SEED,     // [ 0]
			 ...
			 __global float *A_ipFR,   // [ 5]  A_ipFR[NC, NT, NF]
			 __global float *A_ipET,   // [ 6]  A_ipET[NC, NT]
			 __global float *A_ipKABS, // [ 7]  ipKABS[NC, NF]
			 __global float *A_ipKSCA, // [ 8]  ipSCA[NC, NF]
			 __global float *A_ipG,    // [ 9]  ipG[NC, NF]
			 __global float *ABS,      // [10]  ABS[NC]
...
# endif
	    ...
	    iT        =  clamp((int)(round(log(T/T0)/log(KT))), 0, NT-1) ; 
	    //  ipFR[NT, NF]  ->  frequency
	    F         =  A_ipFR[IND*NF*NT+(int)round(iT*NF+NF*Rand(&rng))] ; // random frequency from the lookup table
	    //  frequency -> frequency index,  F =  F0*KF**iF  =>   iF = log(F/F0) / log(KF)
	    ifreq     =  clamp((int)round(log(F/F0)/log(KF)), 0, NF-1) ;	    
	    // update dust parameters -- for the new frequency
	    KABS      =  A_ipKABS[IND*NF+ifreq] ;
...
void __kernel Map(__global float *RC,        //  RC[NC]
		  ...
		  __global float *A_ipKABS,  //  ipKABS[NC, NF]
		  __global float *A_ipKSCA,  //  ipKSCA[NC, NF]
   ...
   int  IND ;
   float x, z, res=0.0f, TAU=0.0f, tau, dx, R, KABS, KSCA ;
   float3 POS, DIR ;
   ...
   while (IND<NC) {                 // *****IGNORING SCATTERING*****
      ...
      // KABS =  A_ipKABS[IND*NF+ifreq] ;   //  tau/H/pc
      // KSCA =  A_ipKSCA[IND*NF+ifreq] ;
      // NOTE !!!! the following ignores optical depth for scattering (<<1 for wvl of non-stochastic emission ???)
      dx   =  Step(&POS, &DIR, IND, RC)  ; 
      tau  =  dx*A_ipKABS[IND*NF+ifreq]*RHO[IND] ;        // optical depth for absorption
