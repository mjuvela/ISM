...
__kernel void Smooth2(
   ...
   float STEP  = 2.0*FWHM_HPF/NX ;
   for(int i=-NX; i<=+NX; i++)  {     // loop over phi
      phi = phi0 + i*STEP/siny ;
                       ...
                       )
{
   ...
   for(int iPA=0; iPA<NDIR; iPA++) {
#if (SYMMETRIC>0)
      pa  = -PIHALF+iPA*PI/NDIR ;
#else
      pa  = iPA*TWOPI/NDIR ;  // full two pi
#endif
      si  =  sin(pa) ;
      co  =  cos(pa) ;
      s   =  0.0f ;    s2 = 0.0f ;
      for(int i=0; i<DIM; i++) {        // towards East
         for(int j=0; j<DIM; j++) {     // towards North
            // North = (-j)*co   ---   (centre-j) increases North  -- the way template defined
            // East  = (-i)*sin  ---   (centre-i) increases East   -- the way template defined
            y        =   co*(centre-j) - si*(centre-i) ;
            theta    =   theta0 - y*FWHM ;    // theta increases down!
            y        =  +si*(centre-j) + co*(centre-i) ;
      ...
      y    =  s/(DIM*DIM) ;                 // mean
#if (STUDENT>0)
      s2   = sqrt(s2-y*y)/DIM ;             // standard deviation
#endif
      // sum of D*S
      s    =  0.0f ;
      for(int i=0; i<DIM*DIM; i++) {             // along the length of the centipede
         s  +=  PAT[i] * (D[i]-y) ;
      }
#if (STUDENT>0)
#if (STUDENT>1)    // between normal and studentized versions
      s /= DIM*DIM*pow(s2, 1.0f/STUDENT) ; // STUDENT>1 ==> less std normalisation
#else
      s /= s2*DIM*DIM ;    // normal case -- divide by std and number of points
#endif
#endif
      if (s>val0) {
         val0 = s ;   pa0 = pa ;
