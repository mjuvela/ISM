...
if (WITH_MSF==0):
    DSC, CSC        =  zeros(USER.DSC_BINS, float32), zeros(USER.DSC_BINS, float32)
else:
    DSC, CSC        =  zeros((NDUST, USER.DSC_BINS), float32), zeros((NDUST, USER.DSC_BINS), float32)    
...
OPT                 =  zeros(3, float32)  # WITH_ABU => large abundance vectors for all dust species
...
ARGS = "-D NX=%d -D NY=%d -D NZ=%d -D BINS=%d -D WITH_ALI=%d \
-D CELLS=%d -D AREA=%.0f -D NO_PS=%d  %s -D WITH_ABU=%d \
-D AXY=%.5ff -D AXZ=%.5ff -D AYZ=%.5ff -D LEVELS=%d -D LENGTH=%.5ef \
-I%s/starformation/pySOC/ -D NSIDE=%d  \
-D POLSTAT=%d -D SW_A=%.3ef -D SW_B=%.3ef -D STEP_WEIGHT=%d -D DIR_WEIGHT=%d -D DW_A=%.3ef \
-D LEVEL_THRESHOLD=%d -D POLRED=%d -D p0=%.4ff -D WITH_COLDEN=%d -D MAXLOS=%.3ef \
-D FFS=%d -D NODIR=%d -D METHOD=%d -D USE_EMWEIGHT=%d -D HPBG_WEIGHTED=%d -D WITH_MSF=%d -D NDUST=%d" % \
(  NX, NY, NZ, USER.DSC_BINS, USER.WITH_ALI,
CELLS, int(USER.AREA), max([1,int(USER.NO_PS)]), USER.kernel_defs, WITH_ABU,
   USER.AXY, USER.AXZ, USER.AYZ, LEVELS, USER.GL*PARSEC, os.getenv("HOME"), USER.NPIX['x'],
   USER.POLSTAT, int(USER.STEP_WEIGHT[0]), USER.STEP_WEIGHT[1], int(USER.STEP_WEIGHT[2]), 
   int(USER.DIR_WEIGHT[0]), USER.DIR_WEIGHT[1],
   USER.LEVEL_THRESHOLD, len(USER.file_polred)>0, USER.p0, len(USER.file_colden)>1, USER.MAXLOS,
   USER.FFS, NODIR, USER.METHOD, USER.USE_EMWEIGHT, USER.HPBG_WEIGHTED, WITH_MSF, NDUST )
NVOPT  = " -cl-fast-relaxed-math"
ARGS   += NVOPT
...
source  =  os.getenv("HOME")+"/starformation/pySOC/kernel_pySOC4_sca.c"        

context, commands =  opencl_init(USER)
program           =  get_program(context, commands, source, ARGS)
mf                =  cl.mem_flags
...
OUT = zeros((NODIR, USER.NPIX['y'], USER.NPIX['x']), float32)
...
for ID in range(DEVICES):
    ...
    EMINDEX_buf.append( cl.Buffer(context[ID], mf.READ_ONLY, 4))
    ...
    if (WITH_MSF==0):
        ...
        G_buf.append(    cl.Buffer(context[ID], mf.READ_ONLY,  4)   )    
        ...
        G_buf.append(    cl.Buffer(context[ID], mf.READ_ONLY,  4*NDUST)  )
...
fp = file('outcoming.socs', 'w')
asarray([USER.NPIX['y'], USER.NPIX['x'], USER.NFREQ], int32).tofile(fp)
asarray(FFREQ, float32).tofile(fp)
fp.close()
OUTCOMING  = \
...
for II in range(3):  # loop over PSPAC, BGPAC and DFPAC==DIFFUSERAD simulations
    ...
    if (II==0):   # point source
        ...
        if (BGPAC<1): continue
        if (len(HPBG)<1): # truely isotropic background
            ...
            BGPAC    =  GLOBAL*BATCH
            PACKETS  =  GLOBAL*BATCH
            Rout     =  sqrt((0.5*NX)**2.0+(0.5*NY**2.0)+(0.5*NZ)**2.0) # radius of a sphere containing the cloud
            WBG      =  4*pi*Rout**2.0*pi/(PLANCK*BGPAC)
    ...
    for IFREQ in range(NFREQ):
        ...
        if (II==0): # update point source luminosoties for the current frequency
        ...
        if ((II==1)&(len(HPBG)>0)):
            ...
            if (USER.HPBG_WEIGHTED):  # using weighted emission from the Healpix background
                ...
                cl.enqueue_copy(commands[ID], HPBG_buf[ID], asarray((WBG/FREQ)*HPBG[IFREQ,:], float32))

                
        print("  FREQ %3d/%3d  %10.3e --  ABS %.3e  SCA %.3e  BG %10.3e  PS %10.3e..." %
        (IFREQ+1, NFREQ, FREQ,    ABS[0], SCA[0], BG, PS[0]))
        ...
        if (II==2): # DFPAC --- the same kernel as for CLPAC !!
            ...
            if ((II==1)&(len(HPBG)>0)): # using a healpix map for the background
                kernel_HP(commands[ID], [GLOBAL,], [LOCAL,],
                PACKETS, BATCH, seed, ABS_buf[ID], SCA_buf[ID], 
                LCELLS_buf[ID], OFF_buf[ID], PAR_buf[ID], DENS_buf[ID], DSC_buf[ID], 
                CSC_buf[ID], NODIR, ODIR_buf[ID], USER.NPIX, USER.MAP_DX,
                USER.MAPCENTRE, RA_buf[ID], DE_buf[ID], OUT_buf[ID], ABU_buf[ID],
                OPT_buf[ID], HPBG_buf[ID], HPBGP_buf[ID])
            else:
                kernel_PB(commands[ID], [GLOBAL,], [LOCAL,],
                np.int32(II), PACKETS, BATCH, seed, ABS_buf[ID], 
                SCA_buf[ID], BG, PSPOS_buf[ID], PS_buf[ID], LCELLS_buf[ID], 
                OFF_buf[ID], PAR_buf[ID], DENS_buf[ID], DSC_buf[ID], CSC_buf[ID], 
