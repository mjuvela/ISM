...
__kernel void Clear(__global float2 *RES) {  // [CELLS] SIJ, ESC
...
__kernel void Update(
                     ...
                     __global float2 *RES      // 13 [CELLS]:  SIJ, ESC
#if (LOC_LOWMEM==1)
...
# define UR 8
      ...
      // Update SIJ and ESC
#if 0
      if ((fabs(nb_nb)>=1.0e-32f))  
        RES[INDEX].x  +=  (A_b/VOLUME) * sum_delta_true /  nb_nb ;
#else
                     ...
                     )
{
   ...
   if (DIR.z>0.9999f) {
      ...
      if (DIR.z<-0.9999f) {
         ...
         // EV  =   DIR z (0,0,1)
         RV.x = DIR.y ; RV.y = -DIR.x ; RV.z = 0.0f ;  RV = normalize(RV) ;
         // DV  =   RV x DIR
   ...
   while (INDEX>=0) {
      ...
      for(i=c1; i<=c2; i++) {         
         ...
         if (fabs(dtau)>1.0e-5f)  {            
            NTRUE[i] += 
              emissivity*profile[i-shift]*GN*exp(-SUM_TAU[i]) * (1.0f-exp(-dtau)) / dtau ;
         } else {
