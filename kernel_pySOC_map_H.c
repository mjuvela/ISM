...
__kernel void Mapping(
                      ...
                      __global   float   *OPT     // 15 OPT = ABS, SCA array float float2
#if (WITH_COLDEN>0)
...
#ifdef USE_ABU
      DTAU    = sx*DENS[oind]*(OPT[2*oind]+OPT[2*oind+1]) ;  // sx in global units
#else
...
#ifdef USE_ABU
      DTAU    = dx*DENS[oind]*(OPT[2*oind]+OPT[2*oind+1]) ;  // dx in global units
#else
...
__kernel void PolHealpixMapping( // HEALPIX POLARISATION MAP
                                 ...
                                 __global   float   *OPT,     // 18 ABS + SCA vectors
...
#ifdef USE_ABU
      DTAU    = sx*dens*(OPT[2*oind]+OPT[2*oind+1]) ;  // sx in global units
#else
