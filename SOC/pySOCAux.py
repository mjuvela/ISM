...
class User:    
    def __init__(self, filename):
        ...
        self.file_emitted     = 'pysoc.emitted'   # file to save emission per cell
        ...
        self.SAVE_INTENSITY = 0      # 1 = save intensity (for DustEM), 2 = save [I, Ix, Iy, Iz]
        ...
        for line in file(filename).readlines():    
            ...
            if (key.find('hpbg')==0):   # Healpix map for intensity at each frequency
                self.file_hpbg = a
                if (len(s)>2): self.HPBG_WEIGHTED = int(s[2])
            ...
            if (key.find('bgpack')==0):      self.BGPAC           =  int(a)
            if (key.find('pspack')==0):      self.PSPAC           =  int(a)
            if (key.find('psmetho')==0):     self.PS_METHOD       =  int(a)
            if (key.find('cellpack')==0):    self.CLPAC           =  int(round(float(a)))
            ...
            if (key.find('local')==0):       self.LOCAL           =  int(a)
            if (key.find('forecefirst')==0): self.FFS             =  int(a)
...
def read_scattering_functions(USER):
    ...
    for i in range(ndust):
        ...
        fp           =  file(USER.file_scafunc[i], 'rb')
...
def OT_cut_levels(infile, outfile, maxlevel, platform=-1):
    ...
    mf       = cl.mem_flags
    LOCAL    =  8
    source   =  file(os.getenv("HOME")+"/starformation/pySOC/kernel_OT_tools.c").read()
    OPT      =  '' 
...
def mmap_intensity(USER, CELLS, vector=False):
    ...
    """
    if (not(USER.SAVE_INTENSITY)): return []        # no intensity file used
    fp =  file("ISRF.DAT", "wb")
...
def temperature_to_emitted(USER, EMITTED):
    ...
    fp    =  file(USER.file_temperature, "rb")
    ...
    for icell in m[0]:       # loop over real cells
        ...
        EMITTED[icell,:] =  (1.0e20*4.0*PI/(PLANCK*USER.FFREQ[a:b])) *  \
        USER.FABS[0][a:b]*Planck(USER.FFREQ[a:b], TNEW[icell])/(USER.GL*PARSEC)   # 1e20*photons/H
    del TNEW
    del m
...
def AnalyseExternalPointSources(NX, NY, NZ, PSPOS, NO_PS):
    """
    For PS_METHOD_B  ==>  PS_METHOD==2
