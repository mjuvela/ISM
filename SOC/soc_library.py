...
MAKE_LIBRARY =  (len(sys.argv)==3)
if (MAKE_LIBRARY): print("*** MAKING A LIBRARY ***")
else:              print("*** USING A LIBRARY ***")
...
if (MAKE_LIBRARY):
    ...
    t000 = 1.0e3*(time.time()-t000)/(float(cells))
    print("*** DIRECT SOLVE %.3f MILLISECONDS PER CELL ***" % t000)
    ...
    if (0):
        for icell in range(CELLS):
            print(" %5.1f %5.1f %5.1f -- %2d %2d %2d -- %5.1f %5.1f %5.1f -- %10.3e" % \
            (x[icell],  y[icell], z[icell],  
            i[icell],   j[icell], k[icell], 
            X[i[icell], j[icell], k[icell]], Y[i[icell], j[icell], k[icell]], Z[i[icell], j[icell], k[icell]],
            EMI[i[icell], j[icell], k[icell], 0]))
        sys.exit()
    ...
    if (CASE in [0, 1, 2, 3]): # PURE PYTHON
        ...
        for icell in range(CELLS):
            if (icell%1000==0): print(" CELL %6d / %6d  --- %5.2f %s" % (icell, CELLS, 100.0*icell/float(CELLS), '%'))
            W    = 0
            S[:] = 0.0
    ...
    t000  =  1.0e3*(time.time()-t000)/(float(CELLS))
    print("*** LIBRARY SOLVE %.3f MILLISECONDS PER CELL ***" % t000)
    
    if (0): # testing - recompute everything cell by cell
        mis = CELLS
        MIS = asarray(arange(CELLS), int32)
    
    # Recompute missing cells one by one
    if (mis<1):
        ...
        fp.close()
        os.system('A2E.py %s missing.dat emitted.add 1' % SOLVER)
        ...
        for s in range(mis):
            ...
            ii, jj, kk         =  i[icell], j[icell], k[icell]
            if (EMI[ii,jj,kk,0]<1e30): # this must have been be a missing entry in the library
