...
2019-03-20:
    Consider replacing simum, remit, wavelengths with masks specified in the optical file:
        ...
        - wavelengths = wavelengths included in the spectrum files
    Also, all files (emitted, spectra) should include the list of the actual
    wavelengths included in the file.
"""
...
-D CELLS=%d -D AREA=%.0f -D NO_PS=%d -D WITH_ABU=%d  -D WITH_COLDEN=%d -D RUSSIAN_ROULETTE=%d \
-D AXY=%.5ff -D AXZ=%.5ff -D AYZ=%.5ff -D LEVELS=%d -D LENGTH=%.5ef -I%s -D EMWEI2_STEP=%d \
...
-D ADHOC=%.8ef %s -D HPBG_WEIGHTED=%d -D WITH_MSF=%d -D NDUST=%d -D OPT_IS_HALF=%d -D POL_RHO_WEIGHT=%d" % \
(  NX, NY, NZ, USER.DSC_BINS, USER.WITH_ALI, USER.PS_METHOD, FACTOR,
   CELLS, int(USER.AREA), max([1,int(USER.NO_PS)]), WITH_ABU, (len(USER.file_colden)>1), USER.RUSSIAN_ROULETTE,
   USER.AXY, USER.AXZ, USER.AYZ, LEVELS, USER.GL*PARSEC, INSTALL_DIR, EMWEI2_STEP,
...
source  =  INSTALL_DIR+"/kernel_SOC.c"        
...
if (len(USER.file_constant_load)>0):
    ...
    print("*** Load constant-source absorptions from %s" % (USER.file_constant_load))
    ...
    for II in range(3):  # loop over PSPAC, BGPAC and DFPAC==DIFFUSERAD simulations
        ...
        for IFREQ in range(NFREQ):
            ...
            print("  === %9.5f seconds" % (time.time()-t0))
        ...
        if (len(CTABS)<1e8):
            print('='*80)
            print("******  CONSTANT   %10s   CTABS -> %12.4e" % (['PS', 'BG', 'DE'][II], mean(CTABS)))
            print('='*80)
...
if (not('SUBITERATIONS' in USER.KEYS)):
    for iteration in range(USER.ITERATIONS):  # iterate over simulation <-> T-update cycles
        ...
        if (CLPAC>0):
            for IFREQ in range(NFREQ):     # loop over single frequencies (all the frequencies)
                ...
                if ((IFREQ<REMIT_I1)|(IFREQ>REMIT_I2)): # this frequency not in EMITTED !!
                    print("*** FREQUENCY NOT IN EMITTED: %d, %.3e  => simulation skipped" % (IFREQ, FREQ))
                ...
                # print("IFREQ %3d, EMIT mean %10.3e max %10.3e" % (IFREQ, mean(EMIT), max(EMIT)))
                ...
                if (USER.USE_EMWEIGHT>0):
                    ...
                    if (skip==3):
                        ...
                        ## print 'REMAIN ', len(nonzero(EMWEI>1e-7)[0])
                        if (USER.USE_EMWEIGHT==2):
                            # Host provides weights in EMWEI and the indices of emitting cells in EMINDEX
                            # Host makes the number of packages divisible by 3 and makes several
                ...
                if (USER.USE_EMWEIGHT==2):
                    ...
                    while 1:
                        ...
                        EMINDEX_buf[ID], OPT_buf[ID], ABU_buf[ID])  # <-- one additional parameter in case of USE_EMWEIGHT==2
                        commands[ID].finish()
                        EMDONE[m] += EMWEI2_STEP  # kernel did EMWEI2_STEP (?) packages for each cell in EMINDEX
                ...
                if (iteration==(USER.ITERATIONS-1)):                    
                    if (USER.SAVE_INTENSITY>0):
                        assert(USER.WITH_REFERENCE==0) ; # we want TABS_buf to contain the total field
                    if ((USER.SAVE_INTENSITY==1)|(not(USER.NOABSORBED))):  # scalar intensity in each cell
                        # note that INT_buf is used to pull absorptions for the current frequency
                        cl.enqueue_copy(commands[ID], TMP, INT_buf[ID])    # ~ total field
                        commands[ID].finish()
                        # print("IFREQ %3d ---TABS mean %10.3e, max %10.3e" % (IFREQ, np.mean(TMP), np.max(TMP)))
                        if (not(USER.NOABSORBED)):                                                        
                            #print("FABSORBED[:,%d] +=  %.3e %.3e %.3e" % (IFREQ, min(TMP), mean(TMP), max(TMP)))
                            FABSORBED[:,IFREQ] += TMP
                        if (USER.SAVE_INTENSITY==1):
                            for level in range(LEVELS):
                                coeff =  KDEV * (PLANCK*FREQ/ABS) * (8.0**level)
            ...
            if (USER.WITH_REFERENCE):
                ...
                if (USER.WITH_ALI):
                    ...
                    if (min(TMP)<0.0):  # @x
                        m = nonzero(TMP<0.0)
                        print("********************************************************************************")
                        print("Invalid escape probabilities:")
                        ttt = -1.0
                        if (len(TNEW)>1): ttt = TNEW[ind]
                        for ind in m[0]:
                            print("IFREQ %d = %.3e   CELL %6d, RHO %10.3e  T %7.3f    XEM %10.3e XAB %10.3e           beta %10.3e" %
                            ( IFREQ, FREQ,   ind, DENS[ind],  ttt, XEM[ind],  EMITTED[ind, IFREQ], TMP[ind]))
                        print("********************************************************************************")                            
                        TMP[m] = 0.5
            ...
            if (USER.NOABSORBED): # WE SOLVE EMISSION WITHIN SOC
                ...
                if (0):
                    m = nonzero(~isfinite(EMIT))
                    print("IFREQ %2d --- KERNEL RETURNED %7d NAN VALUES" % (IFREQ, len(m[0])))
                    EMIT[~isfinite(EMIT)] = 0.0 # ???
                pass
        ...
        #  1e20 * 6.62606957e-27  =  6.62606957e-07  = 1e20 * PLANCK
        scale   = 6.62607e-07/(USER.GL*PARSEC)     # 1.0e20f*PLANCK / (GL*PARSEC).... 1e20 -> FACTOR !!!
        scale  *= FACTOR/1.0e20
        ...
        if (0):  # COMMENTED OUT 2020-07-09
            mmm   = nonzero(~isfinite(EMIT))
            if (len(mmm[0]>0)):
                print('******* EMIT NOT FINITE *******', len(mmm[0]))
            EMIT  = clip(EMIT, 1.0e-25, 1e36)  # was 1e32
        ...
        if (not(USER.NOSOLVE)):
            ...
            if (NDUST>1):
                ...
                sys.exit()
            t0 = time.time()
            if ('CLT' in USER.KEYS):    # calculate temperatures on device
                TTT_buf  = cl.Buffer(context[0], mf.READ_ONLY,  TTT.nbytes)                
                TMP_buf  = None  # for ALI only
                kernel_T = program[0].EqTemperature
                # EMIT + DENS  ->  TABS
                kernel_T.set_scalar_arg_dtypes( \
                #  adhoc     kE          Emin         NE        
                [np.float32, np.float32, np.float32 , np.float32,
                # OFF  LCELLS  TTT    DENS   EMIT   TNEW   TMP 
                None,  None,   None,  None,  None,  None,  None ])
                ###
                cl.enqueue_copy(commands[0], TTT_buf, TTT)
                cl.enqueue_copy(commands[0], EMIT_buf[0], EMIT)
                if (USER.WITH_ALI): 
                    TMP_buf  = cl.Buffer(context[0], mf.READ_ONLY,  TMP.nbytes)  # for ALI
                    cl.enqueue_copy(commands[0], TMP_buf, TMP)
                else:
                    TMP_buf = cl.Buffer(context[0], mf.READ_ONLY, 4)  # dummy buffer
                ###
                kernel_T(commands[0], [GLOBAL,], [LOCAL,], ADHOC, kE, Emin, NE,
                OFF_buf[0], LCELLS_buf[0], TTT_buf, DENS_buf[0], EMIT_buf[0], TABS_buf[0], TMP_buf)
                print('******** TNEW ', len(TNEW))
                ...
                for level in range(LEVELS):
                    ...
                    for i in range(LCELLS[level]):
                        ...
                        told    =  TNEW[ind]
                        # scale includes FACTOR ... back to true photon numbers
                        ...
                        try:
                            ...
                            print("********************************************************************************")
                            print("????? Ein = %12.4e, beta %12.4e, Emin %12.4e, EMIT[%d] = %12.4e .... %12.4e" % 
                            (                  Ein,          beta,        Emin,      ind,    EMIT[ind], (Ein/beta)/Emin))
                            print("********************************************************************************")
                            iE     =  0
                            tnew   =  TTT[0]
            ...
            if (len(TNEW)<1e8):
                ...
                if (len(TNEW)<1e6):
                    print('    TNEW: %.5e %.5e %.5e  ' % tuple(np.percentile(TNEW[mok], (0, 50, 100))))
                print('-'*80)
                print('-'*80)
...
if ('SUBITERATIONS' in USER.KEYS):
    ...
    for iteration in range(USER.ITERATIONS):  # iteratio over simulation <-> T-update cycles
        ...
        # Solve temperatures on the host side -- total absorbed energy in array EMIT[]
        scale  = 6.62607e-07/(USER.GL*PARSEC)     # 1.0e20f*PLANCK / (GL*PARSEC) -> FACTOR !!!
        scale *= FACTOR/1.0e20
        ## oplgkE = 1.0/log10(kE)
        ## ADHOC  = 1.0e-10         # *** SAME AS IN THE KERNEL !! ***
        beta   = 1.0
...
if (not(USER.NOABSORBED)):
    ...
    for level in range(LEVELS):
        ...
        if (0):
            print("FABSORBED MEAN %.3e, MAX %.3e" % (np.mean(FABSORBED[a:b]), np.max(FABSORBED[a:b])))
            clf()
            plot(FABSORBED[a:b:10], 'k.')
            show()
            sys.exit()
        FABSORBED[a:b]  *=  coeff                            # volume scaling
        print("FABSORBED SCALED BY COEFF coeff=%.3e" % coeff) ;
        ...
        if (1):
            ...
            for icell in range(a, b):
                if (DENS[icell]>0.0):   
                    ...
                    FABSORBED[icell,0] = -1.0  # first frequency -1 == tell A2E to skip this one !!
        else: # 2019-02-26
    ...
    if (USER.SAVE_INTENSITY==3):
        ...
        for ifreq in range(NFREQ):            
            ...
            TMP   *=  4.0*pi*1.0e-20    # intensity saved in cgs units ... 4*pi * true intensity
            TPM   *=  1.0e20/FACTOR
...
if ((MAP_SLOW)&(USER.NPIX['y']>0)): # make maps one frequency at a time
    ...
    source      = open(INSTALL_DIR+"/kernel_SOC_map.c").read()
    ...
    KK  = 1.0e3 * PLANCK / (4.0*np.pi) #  1e3 = 1e23/1e20 = remove 1e20 scaling and convert to Jy/sr
    KK *= 1.0e20/FACTOR  # !!!
    KK *= USER.GL*PARSEC
    first_loop = True
...
if ((not(MAP_HIER))&(USER.NPIX['y']<0)): # Healpix map
    ...
    KK  = 1.0e3 * PLANCK / (4.0*np.pi)   #  1e3 = 1e23/1e20 = removing scaling, convert to Jy/sr
    KK *= 1.0e20/FACTOR
    KK *= USER.GL*PARSEC
...
if (MAP_HIER):
    ...
    print("MAP_HIER=%d, NPIX[y]=%d" % (MAP_HIER, USER.NPIX['y']))
    source      = open(INSTALL_DIR+"/kernel_SOC_map_H.c").read()
    program_map = cl.Program(context[0], source).build(ARGS+' -D NSIDE=%d' % (USER.NPIX['x']))
    kernel_map  = None
    if (USER.NPIX['y']<=0):  kernel_map = program_map.HealpixMapping
    else:                    kernel_map = program_map.Mapping
...
if (MAP_FAST):
    ...
    filename    = INSTALL_DIR+"/kernel_SOC_map_X.c"
