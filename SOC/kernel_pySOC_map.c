...
void Index(float3 *pos, int *level, int *ind, 
   ...
   if (*level==0) {   // on root grid
      ...
      *ind = (int)floor((*pos).z)*NX*NY + (int)floor((*pos).y)*NX + (int)floor((*pos).x) ;
      ...
      while ((*level)>0) {
         *ind = PAR[OFF[*level]+(*ind)] ;  *level -= 1 ;  // parent cell index
...
# endif
   ...
   if (*level==0) {   // on root grid
      ...
      *ind = (int)floor((*pos).z)*NX*NY + (int)floor((*pos).y)*NX + (int)floor((*pos).x) ;
      ...
      while ((*level)>0) {
         *ind = PAR[OFF[*level]+(*ind)] ;  (*level)-- ;  // parent cell index
         if ((*level)==0) {       // arrived at root grid
# if (NX>100)
...
__kernel void Mapping(
                      ...
                      __global   float   *OPT     // 15 ABS + SCA, taking into account abundances
#if (WITH_COLDEN>0)
...
#if (WITH_ABU>0)
      DTAU    = sx*DENS[oind]*(OPT[2*oind]+OPT[2*oind+1]) ;
#else
...
__kernel void HealpixMapping(
                             ...
                             const      float3   CENTRE, // 13
                             const      float3   INTOBS, // 14 position of internal observer
                             __global   float   *OPT     // 15 ABS + SCA (if abundance variations)
#if (WITH_COLDEN>0)
...
#if (WITH_ABU>0)
      DTAU    = dx*DENS[oind]*(OPT[2*oind]+OPT[2*oind+1]) ;
#else
...
# endif
   ...
   float p = p0 ;  // constant p0 or encoded in the length of the B vector
...
# else
      // WTF --- Psi is the angle for projected B, just used to calculate Q and U
...
__kernel void PolMapping(
                         ...
                         __global   float   *OPT     // 18 OPT == ABS + SCA for variable abundances
                        )
{
   const int id   = get_global_id(0) ;  // one work item per map pixel
   if (id>=(NPIX.x*NPIX.y)) return ;
   ...
   float sR=0.0f, sJ=0.0f, sRG=0.0f, sJG=0.0f, RQ, RU, JQ, JU, sRP, sJP, d, PR ;
   TAU = 0.0f ;
