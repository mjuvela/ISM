                       ...
                       ) 
{
   ...
   // emission from Pointsources (SOURCE=0) and from isotropic Background (SOURCE=1)
   ...
   for(int III=0; III<BATCH; III++) {
      ...
      if (SOURCE==0) {
         ...
         // Assume unpolarised point source
         STOKES.x  =  PS[level0] ;           //   I  =  PHOTONS  =   ~ Cabs       --- Reissl 2014
         STOKES.y  =  0.0f ;                 //   Q  =  PHOTONS * (Delta C)/C  cos(2*phi)
         STOKES.z  =  0.0f ;                 //   U  = -PHOTONS * (Delta C)/C  sin(2*phi)
         STOKES.w  =  0.0f ;         
...
# endif
      ...
      if (SOURCE==1) {   // background emission -- find out the surface element
         ...
         IndexG(&POS, &level, &ind, DENS, OFF) ;
         // assume unpolarised background
         STOKES.x    =  BG ;        // every packet has equal weight
         STOKES.y    =  0.0f ;
         STOKES.z    =  0.0f ;
         STOKES.w    =  0.0f ;         
      ...
      STOKES     *=  exp(SW_A*free_path-free_path) / SW_A ;
#endif
#if (STEP_WEIGHT==2)
      ...
      STOKES *= 1.0f/(SW_A*SW_B*exp((1.0f-SW_A)*free_path)+2.0f*SW_A*(1.0f-SW_B)*exp((1.0f-2.0f*SW_A)*free_path)) ;
#endif
      steps = 0 ;
      ...
      while(ind>=0) {  // loop until this ray is really finished
         ...
         while(ind>=0) {    // loop until scattering
            ...
            // absorption and scattering cross sections  C and DC needed: for us optical depth per GL, unit density
            // have those as float4 [ ABS, SCA, DABS, DSCA ], as input parameter
            // WITH_ABU =>   float4 array (values for each cell separately)
#if (WITH_ABU>0)
            // dtau      =  ds*DENS[oind]*GOPT(2*oind+1) ;
            dtau      =  ds*DENS[oind]*OPT[oind].y ;   //  ~ C,            OPT.y   =  Q_par + Q_per
            dtauD     =  ds*DENS[oind]*OPT[oind].w ;   //  ~ \Delta C      OPT.w   =  Q_par - Q_per
#else
            // dtau      =  ds*DENS[oind]*(*SCA) ;
            dtau      =  ds*DENS[oind]*OPT.y ;
            dtauD     =  ds*DENS[oind]*OPT.w ;
#endif
