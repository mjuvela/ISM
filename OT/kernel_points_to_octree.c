...
void __kernel SetRootCoordinates(__global float  *CCX,
                                 __global float  *CCY,
                                 __global float  *CCZ,
                                 __global int    *LCELLS) 
{
   ...
   for(int ind=id; ind<LCELLS[0]; ind+=gs) {
      ...
      j  =  (ind/NX) % NY ;  
      k  =   ind / (NX*NY) ;
      CCX[ind] = (i+0.5f) ;      // coordinates in normal SOC coordinates  [0,NX] etc. 
      CCY[ind] = (j+0.5f) ;
      CCZ[ind] = (k+0.5f) ;      // centre of the cell
   }      
}
...
void __kernel Split(const    int      L,          // parent level, L=0, 1, 2, ..., LEVELS-
                    __global float   *PX,         // point coordinates PX[NP]
                    __global float   *PY,         // point coordinates
                    __global float   *PZ,         // point coordinates
                    __global float   *HH,         // hierarchy as a single vector
                    __global float   *CCX,        // CC[].x/y/z centre coordinates of each cell
                    __global float   *CCY,        // CC[cells]
                    __global float   *CCZ,
...
{  
   ...
   for(i=id; i<LCELLS[L]; i+=gs) {      // loop over level L cells
      x    = CCX[OFF[L]+i] ;            // PARENT cell centre in root grid coordinates
      y    = CCY[OFF[L]+i] ;  
      z    = CCZ[OFF[L]+i] ; 
...
{  
   ...
   for(int i=id; i<LCELLS[L]; i+=gs) {  // loop over level L cells
      x    =  CCX[OFF[L]+i] ;           // level L cell centre in root grid coordinates
      y    =  CCY[OFF[L]+i] ;  
      z    =  CCZ[OFF[L]+i] ; 
...
void __kernel AddLevel(const    int     L,        // level
                       ...
                       __global float  *CCX,       // cell centre coordinates
                       __global float  *CCY,
                       __global float  *CCZ,
                       ...
                       __global int    *LCELLS)   // number of cells on each hierarchy level
{
   ...
   d  =   0.25*pow(0.5, L) ;                // one quarter of the parent cell size [root grid units]
   for(int i=id; i<LCELLS[L]; i+=gs) {      // i = level L cell index
      if (HP[i]<=0.0f) {                    // split the parent cell i
         ...
         for(int sid=0; sid<8; sid++) {
            ...
            y +=  (sid%4<=1)   ?  (-d) :  (+d) ;
            z +=  (sid<=3)     ?  (-d) :  (+d) ;
            CCX[OFF[L+1]+ind+sid] = x ;
            CCY[OFF[L+1]+ind+sid] = y ;
            CCZ[OFF[L+1]+ind+sid] = z ;
         }
      }
   }       
}
